...
const Dashboard: React.FC<DashboardProps> = ({ contacts, deals, language, brand, deductTokens }) => {
  ...
  // حساب البيانات المالية الحقيقية
  ...
  const chartData = [
    ...
    { name: 'Current', val: totalCollected },
    { name: 'Projected', val: totalValue },
  ];
  ...
  const t = {
    en: {
      ...
      ticker: 'SYSTEM STATUS: OPTIMAL • NEURAL NETWORK SYNCED • REVENUE VELOCITY ACTIVE • OMNI V3 ONLINE',
    ...
    ar: {
      ...
      ticker: 'حالة النظام: مثالية • الشبكة العصبية متزامنة • سرعة الإيرادات نشطة • أومني V3 متصل',
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex justify-between items-center px-4">
         ...
         <div className={`flex items-center gap-3 px-6 py-2 rounded-full text-[10px] font-black border transition-all ${isSupabaseConfigured ? 'bg-emerald-50 border-emerald-100 text-emerald-600 shadow-lg' : 'bg-slate-100 border-slate-200 text-slate-400'}`}>
            <span className={`w-2 h-2 rounded-full ${isSupabaseConfigured ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}></span>
            {isSupabaseConfigured ? 'Omni Cloud Active' : 'Local Mode'}
         </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-2 bg-slate-950 rounded-[4rem] p-12 text-white border border-white/10 shadow-3xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-6">
               <div className="px-6 py-2 bg-white/5 rounded-full border border-white/10 flex items-center gap-3">
                  ...
                  <span className="text-xs font-black">${totalCollected.toLocaleString()} {language === 'ar' ? 'محصل فعلياً' : 'Collected'}</span>
               </div>
               <div className="text-indigo-400 text-[10px] font-black uppercase tracking-widest">+18.4% Velocity</div>
            </div>
          </div>
          <div className="absolute -right-10 -bottom-10 w-96 h-96 bg-indigo-600/10 rounded-full blur-[100px] group-hover:scale-125 transition-transform duration-1000"></div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[4rem] p-10 border border-slate-100 dark:border-slate-800 shadow-xl flex flex-col items-center justify-center text-center group hover:border-indigo-500 transition-all">
          <div className="w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-[2.5rem] flex items-center justify-center text-4xl text-indigo-600 mb-6 font-black shadow-inner group-hover:scale-110 transition-transform">{collectionRate}%</div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[4rem] p-10 border border-slate-100 dark:border-slate-800 shadow-xl flex flex-col items-center justify-center text-center group hover:border-emerald-500 transition-all">
          <div className="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 rounded-[2.5rem] flex items-center justify-center text-4xl text-emerald-600 mb-6 font-black shadow-inner group-hover:scale-110 transition-transform">{contacts.length}</div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[4.5rem] p-12 border border-slate-100 dark:border-slate-800 shadow-3xl">
         <h3 className="text-xl font-black text-slate-900 dark:text-white mb-12 tracking-tight flex items-center gap-4">
         ...
         <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
               <AreaChart data={chartData}>
                  ...
                  <YAxis hide />
                  <Tooltip 
                    contentStyle={{ borderRadius: '25px', border: 'none', boxShadow: '0 25px 60px rgba(0,0,0,0.1)', fontWeight: 'black', padding: '15px' }} 
         ...
         <div className="relative z-10">
            ...
            {loadingInsight ? (
               ...
               <p className="text-3xl md:text-5xl font-black leading-[1.2] text-white tracking-tighter">
                  "{oracleInsight || 'Injecting strategic mission data...'}"
               </p>
            )}
         </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {priorities.map((p, i) => (
            <div key={i} className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-12 border border-slate-100 dark:border-slate-800 hover:shadow-3xl transition-all hover:-translate-y-3 group cursor-pointer">
              <span className={`text-[10px] font-black px-6 py-2 rounded-full uppercase mb-8 inline-block border ${p.impact === 'High' ? 'bg-rose-50 border-rose-100 text-rose-600 shadow-lg' : 'bg-indigo-50 border-indigo-100 text-indigo-600 shadow-lg'}`}>
                {p.impact} IMPACT
              </span>
