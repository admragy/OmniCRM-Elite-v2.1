...
const Dashboard: React.FC<DashboardProps> = ({ contacts, deals, language, brand, deductTokens }) => {
  ...
  // حساب البيانات الحقيقية للرسم البياني
  const totalValue = deals.reduce((acc, d) => acc + d.value, 0);
  const totalCollected = deals.reduce((acc, d) => {
    return acc + (d.payments?.filter(p => p.status === 'Paid').reduce((sum, p) => sum + p.amount, 0) || 0);
  }, 0);

  const chartData = [
    { name: 'Target', val: totalValue * 0.5 },
    { name: 'Pipeline', val: totalValue * 0.8 },
    { name: 'Collected', val: totalCollected },
  ...
  const t = {
    en: {
      ...
      chartTitle: 'Financial Momentum Radar'
    },
    ar: {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      // تمرير قاعدة المعرفة للأوراكل ليكون رده مخصصاً للبيزنس
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex justify-between items-center px-4">
         <div className="flex flex-col">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{language === 'ar' ? 'مركز العمليات الموحد' : 'Unified Operation Center'}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-2 bg-slate-900 rounded-[3.5rem] p-12 text-white border border-white/5 shadow-3xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 text-emerald-400 font-bold text-sm">
               ...
               <span>${totalCollected.toLocaleString()} {language === 'ar' ? 'محصل فعلياً' : 'Net Collected'}</span>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-10 border border-slate-100 dark:border-slate-800 shadow-xl flex flex-col items-center justify-center text-center group hover:border-indigo-500 transition-all">
          <div className="w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-[2rem] flex items-center justify-center text-4xl text-indigo-600 mb-6 font-black shadow-inner group-hover:scale-110 transition-transform">{collectionRate}%</div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[4rem] p-12 border border-slate-100 dark:border-slate-800 shadow-2xl">
         <h3 className="text-xl font-black text-slate-900 dark:text-white mb-10 tracking-tight flex items-center gap-3">
            <i className="fa-solid fa-tower-observation text-indigo-500"></i>
            {t.chartTitle}
         </h3>
